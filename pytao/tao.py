    ...
    def eles(
        ...
        * type::bpm*  (elements with a type that starts with "bpm")
        ...
        no_slaves : bool, optional
            ...
            (Note: the `Tao.ele_ids` family defaults this to True.)
        track_only : bool, default=False
    ...
    def ele_ids(
        ...
        *selectors : str
        ...
        no_slaves : bool, optional
            If set, multipass_slave and super_slave elements will not
            be included. Defaults to True.
            (Note: `Tao.eles` defaults this to False.)
        track_only : bool, default=False
            Only include elements in the tracking part of the lattice,
            excluding lord elements.
        ...
        list of str or list of ElementID
            A sorted list of canonical element IDs of the form
            `{ix_uni}@{ix_branch}>>{ix_ele}` - as strings by default, or as
            `ElementID` instances when `as_string=False`.
        """
        ...
        def ele_sort(ele: str):
            try:
                ...
                raise ValueError(f"Malformed element ID: {ele}") from None
        ...
        if not selectors:
            ...
            for sel in selectors:
                ...
                for ix_uni in self._expand_universe_spec(uni_part):
                    ele_ids.extend(
                        f"{id_.universe}@{id_.branch}>>{id_.ele_id}"
                        for id_ in self._resolve_element_ids(
                            locator,
                            ix_uni=ix_uni,
                            # An empty branch defers to Tao's own defaulting:
                            # indices match in branch 0, names in all branches.
                            ix_branch="",
                            no_slaves=no_slaves,
                            track_only=track_only,
                        )
                    )
            ids = sorted(set(ele_ids), key=ele_sort)

        if as_string:
            return ids
        return [ElementID.from_tao(ele_id) for ele_id in ids]

    # Back-compat, before when this was limited to 'no_slaves'
    unique_ele_ids_from_branch = ele_ids_from_branch
    unique_ele_ids_from_universe = ele_ids_from_universe
    unique_ele_ids_from_superuniverse = ele_ids_from_superuniverse
    unique_ele_ids = ele_ids
