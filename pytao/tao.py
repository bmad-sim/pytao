...
from .errors import TaoCommandError, TaoInvalidArgumentsError
from .model.config import TaoConfig
from .model.ele import AnyElementID, Comb, Element, ElementID, FillDefault, Which
...
if typing.TYPE_CHECKING:
    # NOTE: keep beamphysics imports in the type checking block!
...
from .interface_commands import _TaoAutogeneratedCommandMixin

logger = logging.getLogger(__name__)


class Tao(_TaoAutogeneratedCommandMixin, TaoCore):
    """
    ...
    plot : str, bool, optional
        ...
        To disable plotting entirely, leave `plot` with its default and set
        `noplot=True`.
    ...
    # Back-compat
    from_lattice_contents = from_contents

    def _check_tao_version(self):
        try:
            version: str | None = self.version()
            version_date: datetime.datetime | None = self.version(as_date=True)
        ...
        if version_date.date() < self._min_tao_version.date():
            logger.warning(
                f"Installed Tao version is lower than pytao's recommended and tested version. "
                f"\n   You have Tao version: {version} ({version_date.date()})"
                f"\n   Recommended version:  {self._min_tao_version.strftime('%Y%m%d')}"
    ...
    def get_comb(
        ...
        *,
        ...
        Comb
            Comb data.
        """
        return Comb.from_tao(
            self,
            ix_uni=ix_uni,
            ix_branch=ix_branch,
            ix_bunch=ix_bunch,
            check_ds_save=check_ds_save,
        )
    ...
    @property
    def temporary_dir(self) -> pathlib.Path:
        """
        The temporary directory used by this Tao instance.

        This is currently only used for particle files that need to exist throughout
        the Tao session for when beam tracking reinitializes.
        ...
        if self._temp_dir is None:
            self._temp_dir = TemporaryDirectory(prefix=f"{type(self).__name__}_{id(self)}")
    ...
    def plot_manager(
        self,
    ) -> BokehGraphManager | NotebookGraphManager | MatplotlibGraphManager:
        """
    ...
    def _get_user_specified_backend(self, backend: str | None):
        if backend is None:
    ...
    def _resolve_ix_uni(self, ix_uni: str | int | None) -> int:
        """
        Resolve a user-provided universe index.

        `None`, `""`, and `-1` all refer to Tao's default universe
        (`s%global%default_universe`), matching Tao's `-1@` convention.
        """
        if ix_uni is None:
            return self.default_universe
        ix_uni = str(ix_uni).strip()
        if ix_uni in {"", "-1"}:
            return self.default_universe
        try:
            return int(ix_uni)
            ...
            raise TaoCommandError(
                f"Invalid universe index {ix_uni!r}: expected an integer, or "
                f"''/-1 for Tao's default universe."
            ) from None

    def _expand_universe_spec(self, spec: str | None) -> list[int]:
        """
        Expand a locator universe prefix to concrete universe indices.

        Follows Tao's universe specification syntax (`tao_pick_universe`):
        `None`/`""`/`"-1"` mean the default universe, `"*"` all universes,
        and `"[2:4,7]"` ranges/lists (brackets optional; `lo:hi:step`
        supported).
        """
        if spec is None:
            return [self.default_universe]
        ...
        if spec in {"", "-1"}:
            return [self.default_universe]
        if spec == "*":
            return list(self.inum("ix_universe"))
            ...
            try:
                if ":" in part:
                    ...
                    if len(bounds) not in (2, 3):
                        raise ValueError
                    lo, hi, *step = bounds
                    universes.extend(range(lo, hi + 1, *step))
                else:
                    universes.append(int(part))
            except ValueError:
                raise TaoCommandError(
                    f"Invalid universe specification {part.strip()!r} in "
                    f"{spec!r}: expected an integer, an integer range "
                    f"(lo:hi), '*', or ''/-1 for Tao's default universe."
                ) from None
        return universes
    ...
    def _resolve_element_ids(
        ...
        *,
        ...
        """
        Resolve an element locator to concrete per-element `ElementID`s via Tao.

        Universe and branch qualifiers inside `ele_id` (e.g. `"1@0>>Q1"`)
        take precedence over the `ix_uni`/`ix_branch` parameters.

        Parameters
        ----------
        ele_id : str
            Tao element locator string.
        ix_uni : str, int, or None
            Universe to use when `ele_id` has no `uni@` prefix.
            `None`, `""`, and `-1` mean Tao's default universe.
        ix_branch : str or int
            Branch to use when `ele_id` has no `branch>>` prefix.
        which : "base", "model", or "design", optional
        no_slaves : bool, default=False
        ...
        list of ElementID
        """
        uni_part, branch_part, elements, negated = split_locator(ele_id)
        if uni_part is None:
            uni_part = "" if ix_uni is None else str(ix_uni).strip()
        if branch_part is None:
            branch_part = str(ix_branch).strip()
        universe = self._resolve_ix_uni(uni_part)
        if negated:
            # In the locator grammar, "~" goes between "uni@" and "branch>>".
            branch_part = f"~{branch_part}"
        ...
        if branch is not None:
            ix_eles = self.lat_list(
                ...
                who="ele.ix_ele",
                ix_uni=uni_part,
                ix_branch=branch_part,
                which=which,
                flags=" ".join(["-array_out", *flags]),
            )
            return [
                ElementID(universe=universe, branch=branch, ele_id=str(ix_ele))
                for ix_ele in ix_eles
            ]
        ...
        ids = []
        for row in rows:
            ix_branch_str, ix_ele_str = row.split(";")
            ids.append(
                ElementID(
                    universe=universe,
                    # ele.ix_branch is real-formatted by Tao (e.g. "1.00000000000000E+00");
                    branch=int(float(ix_branch_str)),
                    ele_id=str(int(ix_ele_str)),
                )
            )
        return ids
    ...
    def eles(
        ...
        * type::bpm*  (elements with a type that starts with "bpm")
        ...
        chamber_walls : bool, optional
            Fill chamber wall data.
        comb : bool, default=False
            Fill comb data.  Comb data is queried once per universe and branch
            and shared among the matching elements.
        comb_data : Comb or None, optional
            Only relevant if `comb=True`.
            If provided, this comb data is used for all elements instead of
            querying Tao.
        control_vars : bool, optional
            Fill control variables.
        cylindrical_map : bool, optional
        ...
        if ele_id is None or (not isinstance(ele_id, int) and not ele_id):
            ids: list[ElementID] = [
                ElementID.from_tao(id_)
                for id_ in self.ele_ids_from_superuniverse(
                    no_slaves=no_slaves,
        ...
        fill_comb = comb is True or (
            isinstance(comb, FillDefault) and defaults and comb.attr in Element.DEFAULTS
        )
        combs: dict[tuple[int, int], Comb] = {}

        def get_comb_data(id_: ElementID) -> Comb | None:
            if comb_data is not None or not fill_comb:
                return comb_data
            assert id_.universe is not None and id_.universe != "*"
            assert id_.branch is not None
            key = (int(id_.universe), int(id_.branch))
            if key not in combs:
                combs[key] = self.get_comb(ix_uni=key[0], ix_branch=key[1])
            return combs[key]
    ...
    # For ele_ids: We can get back either ElementID instances or fully
    # qualified '1@0>>0'-style Tao element names.
    ...
    # Back-compat: these predate the no_slaves=False default above and
    # keep the old no_slaves=True behavior unless overridden.
    unique_ele_ids_from_branch = functools.partialmethod(ele_ids_from_branch, no_slaves=True)
    unique_ele_ids_from_universe = functools.partialmethod(
        ele_ids_from_universe, no_slaves=True
    )
    unique_ele_ids_from_superuniverse = functools.partialmethod(
        ele_ids_from_superuniverse, no_slaves=True
    )
    unique_ele_ids = functools.partialmethod(ele_ids, no_slaves=True)
    ...
    def default_universe(self) -> int:
        ...
        """
        return int(self.universe("")["ix_universe"])

    @default_universe.setter
    def default_universe(self, value: int) -> None:
        value = int(value)
        self.cmd(f"set default universe = {value}")


@contextlib.contextmanager
def _ensure_global_block(tao: Tao, name: str, value: Any, raises: bool = True):
    glob = tao.tao_global()
    orig = glob[name]
    if orig != value:
