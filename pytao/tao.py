...
from .model.ele import AnyElementID, Element, ElementID, Which
...
class _ElementOptions(TypedDict, total=False):
    """Configuration options for `tao.eles`."""

    which: Which
    fill_common: bool
    # Individually fillable elements:
    attrs: bool | None
    bunch_params: bool | None
    chamber_walls: bool | None
    comb: bool | None
    control_vars: bool | None
    floor: bool | None
    grid_field: bool | None
    grid_field_points: bool | None
    lord_slave: bool | None
    mat6: bool | None
    multipoles: bool | None
    orbit: bool | None
    photon: bool | None
    twiss: bool | None
    wake: bool | None
    wall3d: bool | None
    wall3d_table: bool | None
...
class Tao(_TaoAutogeneratedCommandMixin, TaoCore):
    ...
    @overload
    def ele(self, ele_id: AnyElementID, **kwargs: Unpack[_ElementOptions]) -> Element: ...

    @overload
    def ele(
        self, ele_id: Sequence[AnyElementID], **kwargs: Unpack[_ElementOptions]
    ) -> list[Element]: ...

    def ele(
        self,
        ele_id: AnyElementID | Sequence[AnyElementID],
        **kwargs: Unpack[_ElementOptions],
    ) -> Element | list[Element]:
        """
        Create one or more `Element` instances by querying Tao.

        If no keyword arguments are specified, `fill_common` will default to True.
        This fills the most commonly-used element information.

        Alternatively, individual attributes may be opted out by passing
        `False`, or opted in by passing `True`.
        ...
        """
        if not kwargs:
            kwargs = {"fill_common": True}
        if isinstance(ele_id, (int, str, ElementID)):
            return Element.from_tao(self, ele_id, **kwargs)
        return [Element.from_tao(self, id_, **kwargs) for id_ in ele_id]
