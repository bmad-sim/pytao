...
class Tao(_TaoAutogeneratedCommandMixin, TaoCore):
    ...
    _particles_dir: TemporaryDirectory | None
    ...
    def __del__(self):
        try:
            if self._particles_dir is not None:
                self._particles_dir.close()
        except Exception:
            pass
    ...
    def set_initial_particles(
        ...
        *,
        ...
        if self._particles_dir is None:
            self._particles_dir = TemporaryDirectory(
                prefix=f"{type(self).__name__}_{id(self)}"
            )

        temp_file = pathlib.Path(self._particles_dir.name) / f"init_particles_uni_{ix_uni}.h5"
        particles.write(temp_file)
        conf.beam_init.position_file = str(temp_file)
        try:
