...
class Tao(_TaoAutogeneratedCommandMixin, TaoCore):
    ...
    def init_particles(
        ...
        *,
        ...
        """
        Initialize beam tracking using Tao's existing beam_init settings.

        Number of particles and bunch charge are required to generate the bunch.
        Additional parameters may be specified by way of `tao.get_config().beam_init`.
        See below for an example.

        Modifies `beam` `track_end` temporarily to generate particles based on the
        existing settings.
        ...
        bunch_charge : float
            Total charge of the bunch in C.

        Examples
        --------
        Get the initial particles for the first universe - 10 particles, 1e-15
        C charge:

        >>> tao.init_particles(10, 1e-15)
        <ParticleGroup with 10 particles at 0x347866c30>

        Adjust emittance settings and others first with `get_config`:

        >>> bi = tao.get_config().beam_init
        >>> bi.a_norm_emit = 1e-6
        >>> bi.b_norm_emit = 1e-6
        >>> bi.set(tao)
        >>> tao.init_particles(10, 1e-15)
        ...
        Notes
        -----
        For use with multi-universe Tao sessions, you will need to ensure that
        all universes have `n_particle` and `bunch_charge` set prior to calling
        `init_particles` for the first time.  This is because Tao tracks all
        universes at the same time with `set global track_type = beam`.
        """

        ix_uni = str(ix_uni)
        conf = self.get_config(ix_uni=ix_uni)
        conf.beam_init.n_particle = n_particle
        conf.beam_init.bunch_charge = bunch_charge

        # NOTE: track_start can be an empty string, which implies "BEGINNING"
        # But that's not good enough for referring back to that element going forward
        track_start = conf.beam.track_start or "BEGINNING"
        track_start_ele_id = ElementID(universe=int(ix_uni or 1), ele_id=track_start)
        ...
        try:
            ...
            self.track_beam(ix_uni=ix_uni, use_progress_bar=False)
            init_particles = self.particles(track_start_ele_id.tao_string)
        finally:
