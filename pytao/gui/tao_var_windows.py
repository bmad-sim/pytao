...
class tao_var_general_window(tao_list_window):
    ...
    def refresh(self):
        ...
        for item in v1_var_list:
            ...
            tk.Button(
                self.list_frame, text="View...", command=self.open_v1_callback(item[0])
            ).grid(row=i, column=1)
            tk.Button(
                self.list_frame, text="Edit...", command=self.edit_v1_callback(item[0])
            ).grid(row=i, column=2)
            tk.Button(
                self.list_frame,
                text="Delete...",
                command=self.delete_v1_callback(item[0]),
            ).grid(row=i, column=3)
...
class new_v1_frame(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        for ix in range(1, 11):
            tk.Button(self, text="Fill to vars", command=var_fill_callback(ix)).grid(
                row=ix + 3, column=2
            )
        ...
        self.ele_name_button = tk.Button(
            self, text="Browse...", command=lambda: self.lat_browser()
        )
        self.ele_name_button.grid(row=i + 1, column=1, sticky="EW")

        ttk.Separator(self, orient="horizontal").grid(
            row=i + 4, column=0, columnspan=3, sticky="EW"
        )
        i = i + 1
        ...
        self.ix_min = -1
        self.ix_max = -1
        self.var_chooser = ttk.Combobox(
            self,
            textvariable=self.var_ix,
    ...
    def delete(self, ask=True, event=None):
        ...
        ans = messagebox.askokcancel(
            "Delete " + self.name,
            "Delete this v1_array and its associated data?",
            parent=self.parent,
        )
        if not ans:
            return
    ...
    def duplicate(self, event=None):
        ...
        for i in range(len(self.v1_array_wids)):
            if i == 0:
                self.parent.v1_frame_list[-1].v1_array_wids[i].tk_var.set(
                    self.v1_array_wids[i].tk_var.get() + "_copy"
                )
            else:
                self.parent.v1_frame_list[-1].v1_array_wids[i].tk_var.set(
                    self.v1_array_wids[i].tk_var.get()
                )
        ...
        for line in v1_list:
            if line.split(";")[0] == name:
                ...
                tao_message_box(
                    ...
                    message=name
                    + " is already defined.    Would you like to edit its existing properties or overwrite them?",
    ...
    def clone(self, v1_array):
        ...
        for i in range(self.ix_min, self.ix_max + 1):
            ...
            unis = []
            for line in self.pipe.cmd_in(
                "python var " + v1_array + "[" + str(i) + "] slaves"
            ).splitlines():
                u = int(line.split(";")[0])
                if u not in unis:
                    unis.append(u)
