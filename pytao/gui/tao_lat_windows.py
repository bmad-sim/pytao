...
from .util import lat_element, str_to_tao_param
...
class tao_ele_window(tao_list_window):
    ...
    def refresh(self, event=None, ask=True, *args):
        ...
        self.element = lat_element(
            self.ele_wids.uni.get(),
        ...
        tk.Label(self.head_frame, text="Description").grid(row=3, column=2, sticky="W")
...
class tao_multipole_frame(tk.Frame):
        ...
        for line in tao_output[3:]:
            ...
            for item in line[1:]:
                ...
                self.tk_tao_list.append(
                    tk_tao_parameter(
                        str_to_tao_param(name + ";REAL;" + can_vary + ";" + item),
                        self.table_frame,
                        self.pipe,
                    )
                )
        ...
        for i in range(22):  # supports orders 0 through 21
            ...
            if str(i) in self.shown_orders:
                ...
                for j in range(1, len(self.titles)):
                    ...
                    self.tk_tao_list.append(
                        tk_tao_parameter(
                            str_to_tao_param(name + ";REAL;" + can_vary + ";" + str(0)),
                            self.table_frame,
                            self.pipe,
                        )
                    )
    ...
    def check_for_changes(self):
        return check_for_changes(self.tk_tao_list) | check_for_changes(self.top_info)
...
class tao_lattice_window(Tao_Toplevel):
    ...
    def __init__(self, root, pipe, switches="", *args, **kwargs):
        ...
        self.cf_box = tk.OptionMenu(
            self.top_frame, self.col_filter, *col_opts, command=self.col_filter_callback
        )
        ...
        self.ele_list_chooser = tk.OptionMenu(
            self.top_frame, self.ele_list_opt, *ele_list_opts
        )
    ...
    def save_template(self, event=None):
        ...
        self.temp_chooser = tk.OptionMenu(
            self.top_frame,
            self.temp_var,
            *temp_opts,
            ...
            if not ele_list:
                ...
                if (item[0] == "-") & (not arg_switch):
                    ...
                    messagebox.showwarning(
                        "Warning",
                        'No argument given for switch "'
                        + arg_switch
                        + '" (one required).  Skipping this switch...',
                        parent=self,
                    )
                    arg_switch = False
                    continue
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.apply_button = tk.Button(
            self.top_frame, text="Apply Element Names", command=self.apply_callback
        )
    ...
    def _autosize_apply_callback(self):
        ...
        for i in range(ix_min, ix_max + 1):
            if self.parent_type == "data":
                ...
                self.parent.data_dict[i]["ele_" + self.which] = names[
                    (i - ix_min) % len(names)
                ]
            elif self.parent_type == "var":
