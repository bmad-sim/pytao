...
class taoplot:
    ...
    def plot(self, width):
        ...
        gHeightsList.append(
            layout_height * len(gNameList) + layout_height * (len(gNameList) - 1)
        )
        ...
        for gNum in range(1, len(gNameList) + 1):
            ...
            for i in cList:
                cInfo.append(
                    pipe.cmd_in(
                        "python plot_curve " + gFullName + "." + i, no_warn=True
                    ).splitlines()
                )
            ...
            try:
                for i in cList:
                    lInfo.append(
                        pipe.cmd_in(
                            "python plot_line " + gFullName + "." + i, no_warn=True
                        ).splitlines()
                    )
            ...
            try:
                ...
                for i in cList:
                    sInfo.append(
                        pipe.cmd_in(
                            "python plot_symbol " + gFullName + "." + i, no_warn=True
                        ).splitlines()
                    )
                SymbolSuperList = []
            ...
            try:
                ...
                for i in cList:
                    hInfo.append(
                        pipe.cmd_in(
                            "python plot_histogram " + gFullName + "." + i, no_warn=True
                        ).splitlines()
                    )
                hInfoDictList = []
            ...
            for i in range(len(cList)):
                ...
                if cInfoDictList[i]["draw_line"].value is True:  # line width if drawn
                    ...
                    CurveData.append(0)
                CurveData.append(
                    mpl_color(cInfoDictList[i]["symbol"].get_component("color"))
                )  # symbol color

                if (
                    cInfoDictList[i]["symbol"].get_component("type") == "dot"
                    or cInfoDictList[i]["symbol"].get_component("type") == "1"
                    ...
                    FillDict[cInfoDictList[i]["symbol"].get_component("fill_pattern")]
                    == "solid"
                ):
                    CurveData.append(cInfoDictList[i]["symbol"].get_component("color"))
                else:
                ...
                if SymbolsDict[cInfoDictList[i]["symbol"].get_component("type")] != "":
                    CurveData.append(
                        SymbolsDict[cInfoDictList[i]["symbol"].get_component("type")]
                    )  # symbol type
                else:
                    CurveData.append(".")
                CurveData.append(
                    cInfoDictList[i]["symbol"].get_component("line_width")
                )  # symbol line width
            ...
            for i in CurvesList:
                ...
                try:
                    yMax = max(
                        0.5 * max(max(ypList), max(ysList)),
                        2 * max(max(ypList), max(ysList)),
                    )
                    yMin = min(
                        0.5 * min(min(ypList), min(ysList)),
                ...
                if (
                    ...
                    or gInfoDict["graph^type"].value == "wave.0"
                    or gInfoDict["graph^type"].value == "wave.a"
                    or gInfoDict["graph^type"].value == "wave.b"
                ):
                    LineList.append(
                        gSubPlotList[gNum].plot(
                            xpList,
                            ypList,
                            color=i[2],
                            linestyle=i[3],
                    ...
                    if (
                        gInfoDict["graph^type"].value == "wave.0"
                        or gInfoDict["graph^type"].value == "wave.a"
                    ):
                        gSubPlotList[gNum].add_patch(
                            patches.Rectangle(
                                (a1, yMin),
                                a2 - a1,
                                yMax - yMin,
            ...
            if gInfoDict["why_invalid"].value != "":
                raise ValueError(
                    gInfoDict["why_invalid"].value
                    + ", make sure graph is properly initialized"
                )
        ...
        if LatLayout is True:
            ...
            latLayoutSubPlot.add_collection(
                mp.collections.LineCollection(lines, colors=colors, linewidths=widths)
            )
            ...
            latLayoutSubPlot.plot(
                [0, 0], [-1.7 * y_max, 1.3 * y_max], lw=wid, color=color, alpha=0
            )
        ...
        if FloorPlan is True:
            ...
            for i in range(len(fpeInfo)):
                fpeIndexList.append(
                    [int(fpeInfo[i].split(";")[0]), int(fpeInfo[i].split(";")[1])]
                )
            ...
            for i in fpeIndexList:
                fpeCenterDict[str(i)] = [
                    fpeSxDict[str(i)] + (fpeExDict[str(i)] - fpeSxDict[str(i)]) / 2,
                    fpeSyDict[str(i)] + (fpeEyDict[str(i)] - fpeSyDict[str(i)]) / 2,
                ]
                ...
                try:
                    ...
                    # draw line element
                        ...
                        gSubPlotForFloorPlan.add_patch(
                            patches.Rectangle(
                                (
                                    x1 + off2 * np.sin(angStart),
                                    y1 - off2 * np.cos(angStart),
                                ),
                    ...
                    # draw box element
                        ...
                        gSubPlotForFloorPlan.plot(
                            [
                                x1 + off2 * np.sin(angStart),
                                x2 - off1 * np.sin(angStart),
                            ],
                            [
                                y1 - off2 * np.cos(angStart),
                                y2 + off1 * np.cos(angStart),
                            ],
                            lw=width,
                            color=color,
                        )
                        gSubPlotForFloorPlan.plot(
                            [
                                x1 - off1 * np.sin(angStart),
                                x2 + off2 * np.sin(angStart),
                            ],
                            [
                                y1 + off1 * np.cos(angStart),
                                y2 - off2 * np.cos(angStart),
                            ],
                            lw=width,
                            color=color,
                        )
                    ...
                    # draw bow_tie element
                        ...
                        gSubPlotForFloorPlan.plot(
                            [x1, x1 + (x2 - x1) / 2 - off1 * np.sin(angStart)],
                            [y1, y1 + (y2 - y1) / 2 + off1 * np.cos(angStart)],
                            lw=width,
                            color=color,
                        )
                        gSubPlotForFloorPlan.plot(
                            [x1 + (x2 - x1) / 2 - off1 * np.sin(angStart), x2],
                            [y1 + (y2 - y1) / 2 + off1 * np.cos(angStart), y2],
                            lw=width,
                            color=color,
                        )
                        gSubPlotForFloorPlan.plot(
                            [x1, x1 + (x2 - x1) / 2 + off2 * np.sin(angStart)],
                            [y1, y1 + (y2 - y1) / 2 - off2 * np.cos(angStart)],
                            lw=width,
                            color=color,
                        )
                        gSubPlotForFloorPlan.plot(
                            [x1 + (x2 - x1) / 2 + off2 * np.sin(angStart), x2],
                            [y1 + (y2 - y1) / 2 - off2 * np.cos(angStart), y2],
                            lw=width,
                            color=color,
                        )
                    # draw diamond element
                    ...
                    # draw circle element
                        ...
                        gSubPlotForFloorPlan.plot(
                            [
                                ...
                                x1 + off2 * np.sin(angStart - relAngStart),
                            ],
                            [
                                y1 + off1 * np.cos(angStart - relAngStart),
                                y1 - off2 * np.cos(angStart - relAngStart),
                            ],
                            lw=width,
                            color=color,
                        )
                        gSubPlotForFloorPlan.plot(
                            [
                                x2 - off1 * np.sin(angEnd + relAngEnd),
                                x2 + off2 * np.sin(angEnd + relAngEnd),
                            ],
                            [
                                y2 + off1 * np.cos(angEnd + relAngEnd),
                                y2 - off2 * np.cos(angEnd + relAngEnd),
                            ],
                            lw=width,
                            color=color,
                        )
                        ...
                        intersection = intersect(
                            ...
                            line(
                                [
                                    x2 - off1 * np.sin(angEnd),
                                    y2 + off1 * np.cos(angEnd),
                                ],
                                [
                                    x2 + off2 * np.sin(angEnd),
                                    y2 - off2 * np.cos(angEnd + relAngEnd),
                                ],
                            ),
                        )
                    ...
                    if (
                        ...
                        gSubPlotForFloorPlan.text(
                            ...
                            va="center",
                            color="black",
                            rotation=-90 + ((angEnd + angStart) / 2) * conv,
                            clip_on=True,
                    ...
                    if (
                        ...
                        top = [
                            intersection[0] - outerRadius * np.sin(middleAngle),
                            intersection[1] + outerRadius * np.cos(middleAngle),
                        ]
                        bottom = [
                            intersection[0] - innerRadius * np.sin(middleAngle),
                            intersection[1] + innerRadius * np.cos(middleAngle),
                        ]
            ...
            try:
                fbwInfo = pipe.cmd_in(
                    "python building_wall_graph " + gFullName, no_warn=True
                ).splitlines()
                ...
                for i in range(len(fps)):
                    fpsTypeDict[fps[i].split(";")[1].split(":")[0].lower()] = (
                        fps[i].split(";")[2].lower()
                    )
                ...
                for i in fbwCurveList:
                    ...
                    while k > 1:
                        ...
                        if bwnTypeDict[str(i)] not in fpsColorDict.keys():
                            ...
                            print(
                                "No floor_plan shape defined for building_wall segment "
                                + bwnTypeDict[str(i)]
                            )
                            k -= 1
                            continue
                        ...
                        if fbwRadiusList[kIndex] == 0:  # draw building wall line
                            gSubPlotForFloorPlan.plot(
                                ...
                                color=fpsColorDict[bwnTypeDict[str(i)]],
                            )

                        else:  # draw building wall arc
                            centerList = circle_intersection(
                                fbwXList[mIndex],
                                fbwYList[mIndex],
                                fbwXList[kIndex],
                                fbwYList[kIndex],
                            ...
                            mAngle = 360 + conv * np.arctan2(
                                (fbwYList[mIndex] - center[1]),
                                (fbwXList[mIndex] - center[0]),
                            )
                            kAngle = 360 + conv * np.arctan2(
        ...
        fig.tight_layout(pad=0.5)  # prevents graphs from overlapping
