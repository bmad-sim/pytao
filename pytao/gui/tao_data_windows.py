...
class d2_data_frame:
    ...
    def __init__(self, master, root, pipe, d2_data_name, u_ix):
        ...
        for i in range(len(self.d1_data_list)):
            tk.Label(self.frame, text=self.d1_data_list[i]).grid(
                row=i + 1, column=0, sticky="W"
            )
            tk.Button(
                self.frame,
                text="View...",
                command=self.open_d1_callback(
                    self.name,
                    self.d1_data_list[i],
                    pipe,
                    self.d1_ix_lb_list[i],
                    self.d1_ix_ub_list[i],
                    u_ix,
                ),
            ...
            tk.Button(
                self.frame,
                text="Edit...",
                command=self.edit_d2_callback(self.name, pipe),
...
class tao_new_data_window(Tao_Toplevel):
    ...
    def fill_d2_frame(self):
        ...
        # Labels
        self.d2_label_list = [
            tk.Label(self.d2_frame, text="d2_data Name:"),
    ...
    def load_d1_frame(self, ask=True):
        ...
        if c3 and self.name:
            ...
            if ask:
                tao_message_box(
                    ...
                    ans_var,
                    title="Warning",
                    message="Would you like to keep or discard the d1_arrays you defined for "
                    + self.name
                    + "?",
        ...
        if c1:
            if ask:
                ans = messagebox.askyesno(
                    "Warning",
                    name
                    + " already exists as a d2 data array.    Would you like to clone its existing d1_arrays?",
                    parent=self,
                )
            else:
                ans = True
            if ans:
                ...
                clone_dict[name] = self.pipe.cmd_in(
                    "python data_d1_array " + name
                ).splitlines()
        # Clone existing data
        if c2 and (self.clone_d2.get() != name):
            clone_dict[self.clone_d2.get()] = self.pipe.cmd_in(
                "python data_d1_array " + self.clone_d2.get()
            ).splitlines()
        ...
        if clone_dict != {}:
            ...
            tk.Label(self.pw, text="Loading existing d1_arrays").grid(
                row=0, column=0, columnspan=2, sticky="EW"
            )
            ...
            for d1_frame in self.d1_frame_list:
                ...
                for j in range(d1_frame.ix_min, d1_frame.ix_max + 1):
                    ...
                    for p in datum_params:
                        ...
                        if (j in d1_frame.data_dict.keys()) and (
                            p in d1_frame.data_dict[j].keys()
                        ):
...
class new_d1_frame(tk.Frame):
    ...
    def __init__(self, d2_array, name="", full_name=""):
        ...
        # Responses to edits
        self.d1_array_wids[self.d1_array_indices["name"]].tk_wid.bind(
            "<FocusOut>", self.name_handler
        )
        self.d1_array_wids[self.d1_array_indices["ix_min"]].tk_wid.bind(
            "<FocusOut>", self.ix_min_handler
        )
        self.d1_array_wids[self.d1_array_indices["ix_max"]].tk_wid.bind(
            "<FocusOut>", self.length_handler
        )
        self.d1_array_wids[self.d1_array_indices["data_source"]].tk_var.trace(
            "w", self.data_source_handler
        )
        self.d1_array_wids[self.d1_array_indices["data_type"]].tk_var.trace(
            "w", self.data_type_handler
        )
        ...
        self.ele_name_button = tk.Button(
            self, text="Browse...", command=lambda: self.lat_browser("name")
        )
        self.ele_name_button.grid(row=4, column=1, sticky="EW")

        tk.Label(self, text="Start elements:").grid(row=i + 2, column=0, sticky="W")
        self.ele_start_name_button = tk.Button(
            self, text="Browse...", command=lambda: self.lat_browser("start_name")
        )
        self.ele_start_name_button.grid(row=i + 2, column=1, sticky="EW")

        tk.Label(self, text="Ref elements:").grid(row=i + 3, column=0, sticky="W")
        self.ele_ref_name_button = tk.Button(
            self, text="Browse...", command=lambda: self.lat_browser("ref_name")
        )
        ...
        self.ix_max = -1
        self.length = -1
        self.data_chooser = ttk.Combobox(
            self,
            textvariable=self.data_ix,
            values=["PLEASE SPECIFY MIN/MAX INDICES"],
            state="readonly",
        )
        ...
        tk.Label(self, text="Datum Settings:", font="Sans 16").grid(
    ...
    def fill_defaults(self, event=None):
        ...
        self.d1_array_wids[6].tk_var.set(
            True if self.data_dict[self.ix_min]["good_user"] == "T" else False
        )
    ...
    def delete(self, ask=True, event=None):
        ...
        if ask:
            ans = messagebox.askokcancel(
                "Delete " + self.name,
                "Delete this d1_array and its associated data?",
                parent=self.d2_array,
            )
            if not ans:
                return
    ...
    def duplicate(self, event=None):
        ...
        for i in range(len(self.d1_array_wids)):
            if i == 0:
                self.d2_array.d1_frame_list[-1].d1_array_wids[i].tk_var.set(
                    self.d1_array_wids[i].tk_var.get() + "_copy"
                )
            else:
                self.d2_array.d1_frame_list[-1].d1_array_wids[i].tk_var.set(
                    self.d1_array_wids[i].tk_var.get()
                )
    ...
    def make_datum_frame(self, event=None):
        ...
        param_list = [
            ...
            "ele_name;STR;T;",
            "ele_start_name;STR;T;",
            "ele_ref_name;STR;T;",
            "data^merit_type;ENUM;T;",
            "meas_value;REAL;T;",
            "good_meas;LOGIC;T;T",
            "ref_value;REAL;T;",
            "good_ref;LOGIC;T;T",
