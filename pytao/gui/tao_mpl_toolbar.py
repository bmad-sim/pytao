...
class taotoolbar(NavigationToolbar2Tk):
    def __init__(self, canvas_, parent_, width_, GUI_DIR_):
        ...
        self.toolitems = (
            ...
            (
                "Pan",
                "Pan axes with left mouse, zoom with right mouse or scroll wheel",
                "move",
                "pan",
            ),
        ...
        self.template = self.templateDict[
            self.parent.region
        ]  # corresponding template for a region in tao
    ...
    def home(self, *args):
        ...
        try:
            for i in range(len(self.graph_list)):
                self.parent.pipe.cmd_in(
                    "set graph "
                    + self.graph_list[i]
                    + " x.min = "
                    + str(self.gRangeList[i][0]),
                    no_warn=True,
                )
                self.parent.pipe.cmd_in(
                    "set graph "
...
Holding 'x' restricts zooming to the x axis, holding 'y' restricts zooming to the y axis, and holding control maintains the aspect ratio.""",
        ...
        for i in range(len(help_items)):
            tk.Label(win, text=help_items[i]).grid(
                row=2 * i + 1, column=0, sticky="NW", padx=10
            )
            tk.Label(win, text="").grid(row=2 * i + 2, column=0, sticky="W")
            tk.Label(win, text=help_descrips[i], wraplength=wl, justify="left").grid(
                row=2 * i + 1, column=1, sticky="W"
            )
            tk.Label(win, text="", wraplength=wl, justify="left").grid(
                row=2 * i + 2, column=1, sticky="W"
            )
    ...
    def redraw(self):
        ...
        for i in range(len(self.graph_list)):
            ...
            self.parent.pipe.cmd_in(
                "set graph " + self.graph_list[i] + " x.min = " + str(xRange[0]),
                no_warn=True,
            )
            self.parent.pipe.cmd_in(
                "set graph " + self.graph_list[i] + " x.max = " + str(xRange[1]),
                no_warn=True,
            )
            self.parent.pipe.cmd_in(
                "set graph " + self.graph_list[i] + " y.min = " + str(yRange[0]),
                no_warn=True,
            )
            self.parent.pipe.cmd_in(
                "set graph " + self.graph_list[i] + " y.max = " + str(yRange[1]),
                no_warn=True,
            )
