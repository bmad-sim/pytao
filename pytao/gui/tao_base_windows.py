...
class Tao_Toplevel(tk.Toplevel):
    ...
    def destroy(self):
        ...
        for tao_id in self.root.tao_id_list:
            if self.tao_id == tao_id:
                try:
                    self.root.refresh_windows[tao_id].pop(
                        self.root.refresh_windows[tao_id].index(self)
                    )
                except ValueError:
                    pass
    ...
    def __init__(self, parent, tao_list, n_col, pipe, *args, **kwargs):
        ...
        for item in self.tao_list:
            if item.param.name.find("units#") != 0:
                ...
                tk.Label(self, text=item.param.value).grid(
...
class tao_parameter_window(tao_list_window):
    ...
    def __init__(
        self, root, title, tao_list, pipe, plot="", parent=None, *args, **kwargs
    ):
...
class table_window(tao_list_window):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        b2 = tk.Button(
            self.button_frame, text="Discard Changes and Refresh", command=self.refresh
        )
    ...
    def refresh(self):
        ...
        for item in self.bulk_template:
            self.bulk_params.append(
                tk_tao_parameter(copy.copy(item[0]), self.list_frame, self.pipe)
            )
            ...
            self.bulk_apply.append(
                tk.Button(
                    self.list_frame, text="Fill...", command=self.fill_callback(j)
                )
            )
        ...
        for i in range(self.row_num):
            ...
            tk.Button(
                self.list_frame,
                text="View More...",
                command=self.open_detail_window_callback(self.list_rows[i].index),
            ).grid(row=i + 3, column=len(self.list_rows[i].tk_wids))
    ...
    def fill(self, index, event=None):
        ...
        for i in range(len(self.list_rows)):
            self.list_rows[i].tk_tao_params[
                self.bulk_template[index][0].name
            ].tk_var.set(self.bulk_value[index])
    ...
    def apply(self):
        ...
        for i in range(len(self.bulk_params)):
            if self.bulk_filled[i]:
                ...
                tao_set(
                    ...
                    set_str,
                    self.pipe,
                    overide=(self.bulk_params[i].param.type == "LOGIC"),
                )
        ...
        for i in range(len(self.list_rows)):
            ...
            for j in range(len(self.bulk_template)):
                ...
                c1 = (
                    self.list_rows[i].tk_tao_params[name].tk_var.get()
                    != self.bulk_value[j]
                )
                ...
                try:
                    if self.bulk_template[j][0].type == "REAL":
                        c3 = (
                            float(self.list_rows[i].tk_tao_params[name].tk_var.get())
                            != self.list_rows[i].tk_tao_params[name].param.value
                        )
                    elif self.bulk_template[j][0].type == "INT":
                        c3 = (
                            int(self.list_rows[i].tk_tao_params[name].tk_var.get())
                            != self.list_rows[i].tk_tao_params[name].param.value
                        )
                    elif self.bulk_template[j][0].type == "STR":
                        c3 = (
                            str(self.list_rows[i].tk_tao_params[name].tk_var.get())
                            != self.list_rows[i].tk_tao_params[name].param.value
                        )
                    elif self.bulk_template[j][0].type == "LOGIC":
                        c3 = (
                            bool(self.list_rows[i].tk_tao_params[name].tk_var.get())
                            != self.list_rows[i].tk_tao_params[name].param.value
                        )
                except ValueError:
                    c3 = False
    ...
    def open_detail_window(self, index):
        ...
        win = tao_parameter_window(
            self.root, detail_title, self.param_list, self.pipe, parent=self
        )
...
class lw_table_window(Tao_Toplevel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(bulk_format) > 0:
            tk.Button(
                self.button_frame, text="Bulk fill...", command=self.open_bulk_window
            ).pack(side="left")

        self.sh_button = tk.Button(
            self.button_frame, text="Show all", command=self.toggle_show_all
        )
    ...
    def toggle_show_all(self, event=None):
        ...
        newtext = "Show all" if self.hide_nonexist else "Hide nonexistent rows"
    ...
    def bulk_set(self, fill_choices, parent):
        ...
        for i in range(len(self.bulk_format)):
            ...
            if fill_choices[i]["choice"].get() == "formula":
                ...
                len2 = len(self.pipe.cmd_in("python evaluate " + fill_val).splitlines())
                if len1 != len2:
            ...
            if fill_choices[i]["where"].get() == "all":
                ...
                set_str = self.bulk_set_format.format(
            ...
            if fill_choices[i]["choice"].get() == "const":
                ...
                self.pipe.cmd_in(
                    set_str + self.bulk_format[i][0].name + " = " + fill_val
                )
    ...
    def make_bulk_frame(self, parent, bulk_item, usebmd=False):
        ...
        # Title the column
        tk.Label(fill_frame, text=bulk_item.name).grid(
        ...
        none_button = tk.Radiobutton(
            fill_frame, text="", variable=fill_choice, value="none"
        )
        none_button.grid(row=1, column=0, sticky="W")
        tk.Label(fill_frame, text="No Change").grid(row=1, column=1, sticky="W")
        # Constant
        const_button = tk.Radiobutton(
            fill_frame, text="", variable=fill_choice, value="const"
        )
        ...
        if usebmd:
            ...
            bmd_var.set("Base")
            bmd_button = tk.Radiobutton(
                fill_frame, text="", variable=fill_choice, value="bmd"
            )
            bmd_button.grid(row=3, column=0, sticky="W")
            tk.Label(fill_frame, text="From:").grid(row=3, column=1, sticky="W")
            bmd_wid = tk.OptionMenu(
                fill_frame, bmd_var, "Base", "Model", "Design", "Measure", "Reference"
            )
            bmd_wid.grid(row=3, column=2, sticky="EW")
            fill_vars["bmd"] = bmd_var
        # Formula
        if bulk_item.type == "REAL":
            formula_button = tk.Radiobutton(
                fill_frame, text="", variable=fill_choice, value="formula"
            )
            formula_button.grid(row=4, column=0, sticky="W")
            tk.Label(fill_frame, text="Formula:").grid(row=4, column=1, sticky="W")
        ...
        # All
        all_button = tk.Radiobutton(
            where_frame, text="", variable=fill_where, value="all"
        )
        all_button.grid(row=0, column=0, sticky="W")
        tk.Label(where_frame, text="All").grid(row=0, column=1, sticky="W")
        # Range
        range_button = tk.Radiobutton(
            where_frame, text="", variable=fill_where, value="range"
        )
        range_button.grid(row=1, column=0, sticky="W")
        tk.Label(where_frame, text="Range:").grid(row=1, column=1, sticky="W")
...
class tao_branch_widgets:
    ...
    def __init__(self, parent, pipe, default=None):
        ...
        self.branch_chooser = tk.OptionMenu(
            self.parent,
            self.branch_name,
            *self.b_name_list[self.uni.get()],
            command=self.make_ele_chooser,
        )
        self.ele_chooser = ttk.Combobox(
            self.parent,
            textvariable=self.ele,
    ...
    def update(self, event=None):
        ...
        if (
            self.ele.get() not in self.e_name_list[self.uni.get()][self.branch.get()]
        ) & (
            ...
            try:
                if (
                    int(self.ele.get())
                    not in self.e_list[self.uni.get()][self.branch.get()]
                ):
                    messagebox.showwarning(
                        "Error", "Element not found", parent=self.parent
                    )
                    return 0
            except ValueError:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for c in choices:
            ...
            i = i + 1
        self.update_idletasks()
        tk.Label(self, text=message, wraplength=self.winfo_width()).grid(
            row=0, column=0, columnspan=i
        )
...
class ele_shape_frame(tk.Frame):
    ...
    def __init__(self, parent, root, pipe, which):
        ...
        self.title = tk.Label(
            self, textvariable=self.title_text, font=("Sans", 16, "bold")
        )
        ...
        self.move_up_b = tk.Button(
            self.button_frame, text="Move up", command=self.move_up
        )
        self.move_down_b = tk.Button(
            self.button_frame, text="Move down", command=self.move_down
        )
        ...
        self.del_b = tk.Button(
            self.button_frame, text="Delete selected row", command=self.delete_item
        )
        self.new_top_b = tk.Button(
            self.button_frame, text="New at top", command=self.new_at_top
        )
        self.new_bottom_b = tk.Button(
            self.button_frame, text="New at bottom", command=self.new_at_bottom
        )
    ...
    def refresh(self):
        ...
        # Set title
        self.title_text.set(
            "Lat Layout Shapes" if self.type == "lat_layout" else "Floor Plan Shapes"
        )
    ...
    def width_adjust(self, event=None):
        ...
        for j in range(len(widths)):
            ...
            self.shape_table.column(
                self.title_list[j], width=widths[j], minwidth=widths[j]
            )
    ...
    def edit_shape(self, event=None):
        ...
        def write_to_table():
            ...
            tao_set(
                params,
                "set " + self.type + " ele_shape(" + str(current_ix + 1) + ")%",
                self.pipe,
            )
            win.destroy()
            self.refresh()
    ...
    def swap_shapes(self, ix1, ix2):
        ...
        cmd_str = self.shape_set_format.format(
            **dict(zip(self.keys, self.shape_list[ix1]))
        )
        self.pipe.cmd_in(cmd_str)
        cmd_str = self.shape_set_format.format(
            **dict(zip(self.keys, self.shape_list[ix2]))
        )
        self.pipe.cmd_in(cmd_str)
        self.refresh()
    ...
    def duplicate_item(self, event=None):
        ...
        self.pipe.cmd_in(
            "python shape_manage " + self.type + " " + str(ix + 2) + " add"
        )
        # Set the new row to be a copy of the selected row
        self.shape_list[ix][0] = str(ix + 2)
        self.pipe.cmd_in(
            self.shape_set_format.format(**dict(zip(self.keys, self.shape_list[ix])))
        )
        self.refresh()
    ...
    def new_at_bottom(self, event=None):
        ...
        # Add the shape
        self.pipe.cmd_in("python shape_manage " + self.type + " " + str(ix) + " add")
        # Set the ele_id non-empty to make it show up in the table
        cmd_str = self.shape_set_format.format(
            shape_ix=ix,
            ele_id="None",
