...
class tao_beam_init_window(tao_parameter_window):
    ...
    def __init__(self, root, pipe):
        ...
        self.apply_b = tk.Button(self.button_frame, text="Apply", command=self.apply)
        self.apply_b.pack()
    ...
    def refresh(self, event=None):
        ...
        for k in range(len(beam_head_list)):
            beam_head_list[k] = tk_tao_parameter(
                str_to_tao_param(beam_head_list[k]), self.list_frame, self.pipe
            )
        ...
        for k in range(len(beam_body_list)):
            beam_body_list[k] = tk_tao_parameter(
                str_to_tao_param(beam_body_list[k]), self.list_frame, self.pipe
            )
