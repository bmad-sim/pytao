...
from .util import str_to_tao_param, tao_parameter
...
class tk_tao_parameter:
    ...
    def __init__(self, tao_parameter, frame, pipe=0, data_source="", plot="", struct_name=""):
        ...
        if self.param.is_ignored:
            ...
            self._m = ttk.Combobox(
                self.tk_wid,
                textvariable=self._mvar,
                values=self._get_dat_types(True),
                state="readonly",
            )
            ...
            dat_type_str = (
                self.param.name
                + ";DAT_TYPE;"
                + ("T;" if self.param.can_vary else "F;")
                + dat_type_str
            )
            ...
            self._m = tk.Button(
                self.tk_wid, textvariable=self._m_label, command=self._show_hide_struct
            )
            ...
            for component in self.param.value:
                ...
                if self.param.name not in ["x", "x2", "y", "y2"]:
                    self._s.append(
                        tk_tao_parameter(
                            component, self.tk_wid, pipe, struct_name=self.param.name
                        )
                    )
                else:
    ...
    def _s_refresh(self, event=None, dat_source_swap=False, *args):
        ...
        for p in slave_params:
            ...
            if p.find("<enum") != -1:  # Enums
                self._s.append(tk.OptionMenu(self.tk_wid, self._svar[k], *dat_dict[p]))
                ...
                self._s.append(tk.OptionMenu(self.tk_wid, self._svar[k], *digits))
