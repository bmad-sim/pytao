...
class tao_root_window(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.GUI_DIR = (
            os.environ["ACC_LOCAL_ROOT"]
            if "ACC_LOCAL_ROOT" in os.environ.keys()
            else os.environ["ACC_ROOT_DIR"]
        )
    ...
    def tao_call(self, event=None):
        ...
        if self.call_file.tk_var.get() != "Browse...":
            self.pipe.cmd(
                "call " + self.call_file.tk_var.get() + " " + self.cf_args.tk_var.get()
            )
    ...
    def menubar_init(self):
        ...
        create_menu.add_command(
            label="New Data...", command=self.new_data_cmd, accelerator="Ctrl+N"
        )
        create_menu.add_command(
            label="New Variables...", command=self.new_var_cmd, accelerator="Alt+N"
        )
        create_menu.add_command(
            label="New Plot Template...",
            command=self.new_template_cmd,
            accelerator="Alt+P",
        )
        ...
        view_menu.add_command(
            label="Data...", command=self.view_data_cmd, accelerator="Ctrl+D"
        )
        view_menu.add_command(
            label="Variables...", command=self.view_vars_cmd, accelerator="Alt+V"
        )
        view_menu.add_command(
            label="Lattice Elements...", command=self.view_ele_cmd, accelerator="Ctrl+E"
        )
        view_menu.add_command(
        ...
        view_menu.add_command(
            label="Bmad Parameters...",
            command=self.set_bmad_com_cmd,
            accelerator="Alt+G",
        )
        ...
        plot_menu.add_command(
            label="New Plot...", command=self.plot_template_cmd, accelerator="Ctrl+T"
        )
        plot_menu.add_command(
            label="Edit Plot...", command=self.plot_region_cmd, accelerator="Ctrl+R"
        )
        plot_menu.add_command(
            label="Building Wall Settings...", command=self.building_wall_cmd
        )
        self.menubar.add_cascade(label="Plot", menu=plot_menu)

        beam_menu = tk.Menu(self.menubar, tearoff=0)
        beam_menu.add_command(
            label="Beam settings...", command=self.beam_init_cmd, accelerator="Ctrl+B"
        )
    ...
    def tao_load(self, init_frame):
            ...
            if looking_for == "switch":
                ...
                if arg[0] == "-":  # -switch
                    ...
                    if len(matches) == 1:  # exactly one match -> good switch
                        arg = matches[0]
                        if (tao_startup_param_dict[matches[0]].type == "FILE") & (
                            i != len(sys.argv) - 1
                        ):
        ...
        for param, tao_param in tao_startup_param_dict.items():
            ...
            if param == "noinit":  # gets a special label
                tk.Label(init_frame, text="Don't use an init file (noinit)").grid(
                    row=k, sticky="E"
                )
        ...
        # Add separator
        tk.ttk.Separator(init_frame, orient="horizontal").grid(
            row=k, columnspan=2, sticky="ew"
        )
        k = k + 1
        ...
        tk.OptionMenu(
            init_frame, interface_to_tao, "pexpect", "ctypes", command=swap_box
        ...
        if "plot_mode" in init_dict:
            if init_dict["plot_mode"] == "pgplot":
                ...
                messagebox.showwarning(
                    "Error",
                    'Bad setting of "plot_mode" in gui.init: ' + init_dict["plot_mode"],
                )
        ...
        if ("do_mainloop" in init_dict.keys()) and (
            init_dict["do_mainloop"] in ["F", "False"]
        ):
    ...
    def reinit_cmd(self):
        ...
        result = messagebox.askquestion(
            "Reinit", "This will close all windows and restart Tao.  Are you sure?"
        )
        if result != "yes":
            return
    ...
    def debug_cmd(self):
        ...
        i = 1
        for o in opts:
            tk.Radiobutton(win, text=o, variable=var, value=str(i)).grid(
