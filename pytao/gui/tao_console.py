...
class tao_console(tk.Frame):
    ...
    def __init__(self, parent, root, pipe):
        ...
        # Scrollbar
        self.scrollbar = tk.Scrollbar(self, orient="vertical", command=self._wid.yview)
        ...
        self.show_output(
            self.pipe.exe_lib_warnings,
            mode=self.pipe.exe_lib_warning_type,
            noprompt=True,
        )
    ...
    def set_command(self, new_command):
        ...
        self._wid.delete(self.cstart, self.cstart + "+" + str(len(self.command)) + "c")
        ...
        self._wid.mark_set("insert", self.cstart + "+" + str(self.cpos) + "c")
    ...
    def _bs_handler(self, event):
        if self._curs_check():
            ...
            self.cpos -= 1
            self.command = self.command[: self.cpos] + self.command[self.cpos + 1 :]
            self._wid.see(self.cstart)
        return "break"
    ...
    def _l_handler(self, event):
        ...
        curs_col = self._get_curs_c()
        if self._curs_check():
            self._wid.mark_set("insert", str(curs_line) + "." + str(curs_col - 1))
            self.cpos -= 1
        return "break"
