...
# Generated on: 2024-08-07 14:22:24
# ==============================================================================
...
def test_constraints_1(caplog, tao_cls):
    with ensure_successful_parsing(caplog):
        with new_tao(
            tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_optics_matching"
        ) as tao:
...
def test_ele_grid_field_1(caplog, tao_cls):
    with ensure_successful_parsing(caplog):
        with new_tao(
            tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_grid"
        ) as tao:
            tao.ele_grid_field(
                ele_id="1@0>>1", which="model", index="1", who="base", verbose=True
            )
...
def test_ele_param_1(caplog, tao_cls):
    with ensure_successful_parsing(caplog):
        with new_tao(
            tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_photon"
        ) as tao:
...
def test_ele_wake_1(caplog, tao_cls):
    with ensure_successful_parsing(caplog):
        with new_tao(
            tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_wake"
        ) as tao:
