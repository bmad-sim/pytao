...
def test_ele_gen_gradients_1(caplog, tao_cls):
    with ensure_successful_parsing(caplog):
        with new_tao(
            tao_cls,
            "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_em_field",
...
def test_ele_wake_2(caplog, tao_cls):
    with ensure_successful_parsing(caplog):
        with new_tao(
            tao_cls,
            "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_wake",
            ...
            tao.ele_wake(ele_id="zw", which="model", who="sr_z_long_table", verbose=True)
