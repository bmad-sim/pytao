...
def test_building_wall_list_1(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_wall"
    ) as tao:
...
def test_constraints_2(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/cesr/tao.init"
    ) as tao:
...
def test_ele_chamber_wall_1(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_wall3d"
    ) as tao:
        assert set(
...
def test_ele_gen_grad_map_1(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_em_field"
    ) as tao:
        assert set(
...
def test_ele_taylor_1(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_taylor"
    ) as tao:
...
def test_ele_spin_taylor_1(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_spin"
    ) as tao:
...
def test_floor_orbit_1(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_floor_orbit"
    ) as tao:
...
def test_shape_pattern_list_1(tao_cls):
    with new_tao(
        tao_cls, "-init $ACC_ROOT_DIR/regression_tests/pipe_test/tao.init_shape"
    ) as tao:
...
def test_show_1(tao_cls):
    pytest.skip("TODO")
    tao = new_tao("-init $ACC_ROOT_DIR/regression_tests/pipe_test/cesr/tao.init")
