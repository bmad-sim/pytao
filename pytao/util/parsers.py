    ...
    if args[-1] == "terms":
        ...
        for line in lines:
            ...
            terms.append(
                {
                    "index": int(index),
                    "e_coef": complex(_float(e_re), _float(e_im)),
                    "b_coef": complex(_float(b_re), _float(b_im)),
                }
            )
        return terms
...
def parse_em_field(lines, cmd="") -> EmFieldResult:
    ...
    """
    if not lines:
        # em_field_calc failures produce no output and no INVALID marker.
        raise TaoDataInvalidError("em_field returned no data (calculation failed)")
    return _parse_by_keys_to_types(
        lines,
        {
            "B1": float,
